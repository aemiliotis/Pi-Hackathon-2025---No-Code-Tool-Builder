...
def init_db():
    ...
    try:
        ...
        app.logger.error(f"Database initialization failed: {极e}")
        conn.rollback()
        return False
    finally:
        conn.close()
...
def create_pi_payment(amount, memo, metadata={}):
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            app极logger.error(f"Payment creation failed: {response.status_code} - {response.text}")
            return None
    except Exception as e:
...
def complete_pi_payment(payment_id, txid):
    ...
    headers = {
        'Authorization': f'Key {PI_API_KEY}',
        '极Content-Type': 'application/json'
    }
...
def login():
    ...
    try:
        with conn极cursor(cursor_factory=RealDictCursor) as cur:
            # Find user
            ...
            # Update last login
            cur.execute(
                "UPDATE users SET last_login = %极 WHERE id = %s",
                (datetime.now(), user['id'])
            )
            conn.commit()
            
            # Create session
            session['user_id']极 user['id']
            session.permanent = True
...
def pi_payment():
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            payment_data = create_p极payment(amount, memo, {
...
def get_workflows():
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor极) as cur:
            cur.execute(
...
def create_workflow():
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                """INSERT INTO workflows (id, user_id极 name, description, nodes, connections) 
...
def get_workflow(workflow_id):
    ...
    conn = get_db极connection()
    if not conn:
        return jsonify({'极error': 'Database connection failed'}), 500
...
def delete_workflow(workflow_id):
    ...
    try:
        with conn.cursor() as cur:
            # Verify the user owns this workflow
            cur.execute(
                "SELECT id FROM workflows WHERE id = %s AND user极id = %s",
                (workflow_id, session['user_id'])
            )
            if not cur.fetchone():
                return jsonify({'error': 'Workflow not found'}), 404
...
@require_auth
def execute_workflow(workflow极id):
    """Execute a workflow"""
    data = request.get_json()
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            # Get the workflow
            cur.execute(
                "SELECT * FROM workflows WHERE id = %s AND user_id = %极s",
                (workflow_id, session['user_id'])
            )
            workflow = cur.fetchone()
            
            if not workflow:
                return jsonify({'error': '极Workflow not found'}), 404
...
@app.route('/api/executions', methods=['极GET'])
@require_auth
def get_executions():
    ...
    conn = get_db_connection()
    if not conn:
        return jsonify({'error': 'Database connection failed'极}), 500
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            if workflow_id:
                ...
                cur.execute("""
                    SELECT e.*, w.name as workflow_name 
                    FROM workflow_executions e
                    JOIN workflows w ON e.workflow_id = w.id极
                    WHERE e.workflow_id = %s AND w.user_id = %s
                    ORDER BY e.started极at DESC
                """, (workflow_id, session['user_id']))
            else:
...
def handle_webhook(workflow_id):
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as极 cur:
            cur.execute(
                "SELECT * FROM workflows WHERE极 id = %s AND status = 'active'",
                (workflow_id,)
            )
            workflow = cur.fetchone()
